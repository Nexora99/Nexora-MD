...
function attachCommandHandler(sock) {
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const without = text.slice(1).trim();
        if (!without) return;
        ...
        try {
            ...
            if (cmd.execute.length >= 3) await cmd.execute(wrappedSock, msg, args);
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "close") {
                ...
                try {
                    ...
                    if (me) {
                        const ownerJid = `${me}@s.whatsapp.net`;
                        global.ownerJid = ownerJid;
                        await sock.sendMessage(ownerJid, {
                            text: `✅ Nexora connected successfully.\nNumber: +${me}`
                        });
                    }
                } catch (e) {
